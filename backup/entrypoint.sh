...
: "${STANZA:?required env var STANZA not set (see .env.example)}"
...
# Full backup
${BACKUP_FULL_CRON}  postgres  PGBACKREST_CIPHER_PASS='${PGBACKREST_CIPHER_PASS}' pgbackrest --stanza=${STANZA} --type=full backup --log-level-console=info

# Differential backup
${BACKUP_DIFF_CRON}  postgres  PGBACKREST_CIPHER_PASS='${PGBACKREST_CIPHER_PASS}' pgbackrest --stanza=${STANZA} --type=diff backup --log-level-console=info

# Health check
${BACKUP_CHECK_CRON}  postgres  PGBACKREST_CIPHER_PASS='${PGBACKREST_CIPHER_PASS}' pgbackrest --stanza=${STANZA} check --log-level-console=info
CRON
